...
import { FLOW, SCOPE_LABELS } from '@/data/vocab';
...
export function suggestWard(request: Pick<BedRequest, 'site' | 'needs'>, wards: Ward[]): PlacementSuggestion {
  ...
  const noBed = (): PlacementSuggestion => ({ note: 'no-bed', label: FLOW.placement.noBed(SCOPE_LABELS[otherSite(request.site)]) });
  ...
  if (any) return { ward: any, note: 'utlokalisering', label: `${any.name} – ${FLOW.placement.relocation.toLowerCase()}` };
  return noBed();
}
