...
import { CURRENT_USER, SYSTEM_ACTOR } from '@/data/vocab';
 ...
 */
export function MessageThread({ messages, onSend, onCreateRequest, emptyText = t('INCIDENT.noMessages'), className }: MessageThreadProps) {
        ...
        {messages.map((m) => {
          ...
          return (
            <div key={m.id} className={cn('flex flex-col', sent ? 'items-end' : 'items-start')}>
              ...
              <div lang={sent ? undefined : 'sv'} className={cn('max-w-[70%] rounded-lg px-3 py-2 text-body', sent ? 'bg-primary text-primary-foreground' : 'bg-bg-muted text-text')}>{m.text}</div>
