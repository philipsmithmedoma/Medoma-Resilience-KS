...
const COLOURS = { arrivals: '#EAECED', admissions: '#186CE9', discharges: '#1AA339', elective: '#BB6BD9', free: '#000000', deficit: '#E33026' };
...
export function ForecastChart({ result, clock }: { result: ForecastResult; clock: number }) {
  ...
  const barW = slot / 5;
  const series: Array<{ key: keyof typeof COLOURS; label: string; get: (b: ForecastResult['buckets'][number]) => number }> = [
  ...
  return (
    <div>
      <div className="mb-2 flex flex-wrap items-center gap-4 text-small text-text-secondary">
        {series.map((s) => (
          <span key={s.key} className="flex items-center gap-1.5">
            <span className="inline-block size-3 rounded-sm" style={{ backgroundColor: COLOURS[s.key], border: s.key === 'arrivals' ? '1px solid #CCCCCC' : undefined }} />
            {s.label}
          </span>
        ))}
      ...
      <svg viewBox={`0 0 ${W} ${H}`} className="h-auto w-full" role="img" aria-label={FLOW.forecast.title}>
        <line x1={PAD.left} x2={W - PAD.right} y1={yFree(0)} y2={yFree(0)} stroke="#8B8B8B" strokeWidth={1} strokeDasharray="4 4" />
        {buckets.map((b, i) => (
          <g key={b.start}>
            {series.map((s, j) => {
              const v = s.get(b);
              const x = PAD.left + i * slot + barW / 2 + j * barW;
              return <rect key={s.key} x={x} y={yBar(v)} width={barW - 1} height={Math.max(0, PAD.top + innerH - yBar(v))} fill={COLOURS[s.key]} stroke={s.key === 'arrivals' ? '#CCCCCC' : undefined} />;
            })}
            {i % 2 === 0 ? (
              <text x={PAD.left + i * slot + slot / 2} y={H - 8} textAnchor="middle" fontSize={12} fill="#5A5A5A">
                {b.label}
              </text>
            ) : null}
          </g>
        ...
        {nowIndex >= 0 ? (
          <g>
            <line x1={PAD.left + nowIndex * slot + slot / 2} x2={PAD.left + nowIndex * slot + slot / 2} y1={PAD.top} y2={PAD.top + innerH} stroke="#186CE9" strokeWidth={1} strokeDasharray="2 3" />
            <text x={PAD.left + nowIndex * slot + slot / 2 + 4} y={PAD.top + 12} fontSize={12} fill="#186CE9">
              {FLOW.forecast.now}
            </text>
          </g>
        ) : null}
        <polyline points={linePoints} fill="none" stroke={COLOURS.free} strokeWidth={2} />
        {buckets.map((b, i) => (
          <circle key={b.start} cx={PAD.left + i * slot + slot / 2} cy={yFree(b.free)} r={3} fill={b.free < 0 ? COLOURS.deficit : COLOURS.free} />
        ))}
        <text x={W - PAD.right + 6} y={yFree(freeMax) + 4} fontSize={12} fill="#5A5A5A">
          {fmt(Math.round(freeMax))}
        </text>
        <text x={W - PAD.right + 6} y={yFree(0) + 4} fontSize={12} fill="#5A5A5A">
          0
        </text>
        {freeMin < 0 ? (
          <text x={W - PAD.right + 6} y={yFree(freeMin) + 4} fontSize={12} fill="#E33026">
            {fmt(Math.round(freeMin))}
          </text>
        ) : null}
        <text x={PAD.left - 6} y={PAD.top + 4} textAnchor="end" fontSize={12} fill="#5A5A5A">
          {fmt(Math.round(barMax))}
        </text>
        <text x={PAD.left - 6} y={PAD.top + innerH + 4} textAnchor="end" fontSize={12} fill="#5A5A5A">
          0
        </text>
      </svg>
    </div>
