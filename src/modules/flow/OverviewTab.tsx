...
import { FLOW, FLOW_BLOCK_LABELS, SITE_LABELS } from '@/data/vocab';
import { aggregateFigure, blockStatus, formatRowValue, FLOW_ROWS, sumMetric, type FlowRow, type SiteValues } from '@/lib/flow';
...
function cellText(row: FlowRow, figures: Record<string, Figure> | undefined): string {
  ...
  if (row.composite) {
    ...
    if (row.key === 'ct.down') return FLOW.ctDown(fmt(parts[0]), fmt(parts[1]));
  ...
  return (
    <div className="space-y-8">
      {BLOCKS.map((block) => {
        ...
        return (
          <section key={block} aria-label={FLOW_BLOCK_LABELS[block]}>
            <div className="mb-2 flex items-center gap-3">
              <h2 className="text-heading">{FLOW_BLOCK_LABELS[block]}</h2>
              <StatusChip status={status} label={FLOW.status[status]} />
            ...
            <table className="w-full text-body">
              <thead>
                <tr className="text-small text-text-secondary">
                  <th className="h-8 text-left font-normal">{FLOW.columns.metric}</th>
                  {multi ? (
                    <>
                      <th className="h-8 w-40 text-right font-normal">{SITE_LABELS.solna}</th>
                      <th className="h-8 w-40 text-right font-normal">{SITE_LABELS.huddinge}</th>
                      <th className="h-8 w-40 text-right font-normal">{FLOW.columns.sum}</th>
                    </>
                  ) : (
                    <th className="h-8 w-40 text-right font-normal">{SITE_LABELS[sites[0]]}</th>
                  )}
              ...
              <tbody>
                {rows.map((row) => {
                  ...
                  const qualifier = row.qualifierKey
                    ? multi
                      ? `${FLOW.blocksQualifier.ofWhich} ${fmt(sumMetric(row.qualifierKey, values))} ${FLOW.blocksQualifier.asih}`
                      : `${FLOW.blocksQualifier.ofWhich} ${fmt(values[0]?.figures[row.qualifierKey]?.value ?? null)} ${FLOW.blocksQualifier.asih}`
  ...
  return (
    <Popover>
      ...
      <PopoverContent align="end" className="w-96 text-body">
        <h3 className="mb-1 font-medium">{FLOW.staffingTitle}</h3>
        <p>{FLOW.staffingPopover(vacant, agency)}</p>
      </PopoverContent>
    </Popover>
  );
}
