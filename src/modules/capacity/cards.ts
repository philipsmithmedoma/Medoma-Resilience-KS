...
export function scopeCards({ scope, nodes, capabilities, flowMetrics, outage }: CardInputs): CardModel[] {
  ...
  if (sites.length === 0) {
    ...
    return [
      {
        key: 'beds',
        title: t('CAP.cards.places'),
        tile: tile(BedIcon, 'beds'),
        figure: node.beds ? viewFigure(node.beds.free, outage) : undefined,
        unit: node.beds ? t('CAP.units.ofTotal', { total: fmt(node.beds.total.value) }) : undefined,
        notAvailable: !node.beds,
      },
      { key: 'intensive', title: t('CAP.cards.intensive'), tile: tile(HeartPulseIcon, 'intensive'), notAvailable: true },
      { key: 'theatres', title: t('CAP.cards.theatres'), tile: tile(ScissorsIcon, 'theatres'), notAvailable: true },
      { key: 'imaging', title: t('CAP.cards.imaging'), tile: tile(ScanIcon, 'imaging'), notAvailable: true },
      { key: 'ed', title: t('CAP.cards.ed'), tile: tile(SirenIcon, 'ed'), notAvailable: true },
      { key: 'staff', title: t('CAP.cards.staff'), tile: tile(UsersIcon, 'staff'), figure: node.staffOnDuty ? viewFigure(node.staffOnDuty, outage) : undefined, unit: t('CAP.units.staff'), notAvailable: !node.staffOnDuty },
    ];
  }
  ...
  const subtitle = region ? t('CAP.cards.network') : undefined;
  const karolinskaOnly = region ? `(${'Karolinska'})` : undefined;

  return [
    {
      key: 'beds',
      title: t('CAP.cards.beds'),
      subtitle,
      tile: tile(BedIcon, 'beds'),
      figure: beds,
      unit: t('CAP.units.ofDisponibla', { total: fmt(bedsTotal?.value ?? null) }),
      detail: region ? { kind: 'nodes' } : { kind: 'capability', capabilityKind: 'beds' },
    },
    {
      key: 'intensive',
      title: t('CAP.cards.intensive'),
      subtitle: karolinskaOnly,
      tile: tile(HeartPulseIcon, 'intensive'),
      figure: icu,
      unit: t('CAP.units.ofTotal', { total: fmt(icuTotal?.value ?? null) }),
      detail: { kind: 'capability', capabilityKind: 'intensive' },
    },
    {
      key: 'theatres',
      title: t('CAP.cards.theatres'),
      subtitle: karolinskaOnly,
      ...
      unit: t('CAP.units.surgeries', { theatres: fmt(theatresFree) }),
      detail: { kind: 'capability', capabilityKind: 'surgery' },
    },
    {
      key: 'imaging',
      title: t('CAP.cards.imaging'),
      subtitle: karolinskaOnly,
      ...
      unit: t('CAP.units.scanners', { total: fmt(ctTotal), waiting: fmt(ctWaiting?.value ?? null) }),
      detail: { kind: 'capability', capabilityKind: 'ct' },
    },
    {
      key: 'ed',
      title: t('CAP.cards.ed'),
      subtitle: karolinskaOnly,
      tile: tile(SirenIcon, 'ed'),
      figure: ed,
      unit: t('CAP.units.edPatients', { waiting: fmt(edWaiting?.value ?? null) }),
      detail: { kind: 'popover', text: `${t('CAP.cards.ed')}: ${fmt(ed?.value ?? null)}, ${t('CAP.units.edPatients', { waiting: fmt(edWaiting?.value ?? null) })}.` },
    },
    {
      key: 'staff',
      title: t('CAP.cards.staff'),
      subtitle: karolinskaOnly,
      tile: tile(UsersIcon, 'staff'),
      figure: staff ? { ...staff, confidence: weakest(staff.confidence, 'illustrative') } : undefined,
      unit: t('CAP.units.staff'),
    },
  ];
}
